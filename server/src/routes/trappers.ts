...
router.post('/', async (req: Request, res: Response) => {
  const { name, description, trapId, destinationUrl, retryPolicy, authType, authValue, rateLimit, rateLimitWindowMs, hmacSecret, hmacHeader, hmacAlgorithm, overrideEnabled, overridePayload } = req.body;
...
router.put('/:id', async (req: Request, res: Response) => {
  const trapper = db.select().from(trappers).where(eq(trappers.id, req.params.id)).get();
  if (!trapper) { res.status(404).json({ error: 'Not found' }); return; }
  const { name, description, destinationUrl, retryPolicy, authType, authValue, rateLimit, rateLimitWindowMs, hmacSecret, hmacHeader, hmacAlgorithm, overrideEnabled, overridePayload } = req.body;
  const [updated] = db.update(trappers)
    .set({ name, description, destinationUrl, retryPolicy, authType, authValue, rateLimit, rateLimitWindowMs, hmacSecret, hmacHeader, hmacAlgorithm, overrideEnabled: overrideEnabled ? 1 : 0, overridePayload: overridePayload ?? null, updatedAt: new Date().toISOString() })
